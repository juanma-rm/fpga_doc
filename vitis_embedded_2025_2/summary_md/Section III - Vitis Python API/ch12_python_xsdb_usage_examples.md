...
1. **Use workspace journal for replay** — Every IDE action is logged to `workspace_journal.py`; use it as a starting point for automation scripts.
2. **Use `vitis -s` for CI/CD** — Batch mode (`vitis -s <script.py>`) is ideal for regression testing and automated builds.
3. **Use target filters, not IDs** — Target IDs change between sessions; use `filter="name =~ ..."` for robust scripts.
4. **Use Target objects for multi-target debug** — Target objects (from `session.targets(n)`) allow independent operations without switching context.
5. **Always `time.sleep()` after FSBL** — Give FSBL adequate time to complete initialization before downloading the next binary.
6. **Use `memmap` for attach-to-running** — Map symbol files with `memmap()` to debug programs already running on the target.
7. **Disable security gates on ZynqMP** — Write `0x1ff` to `0xffca0038` before accessing PMU or other secure targets.
8. **Use `dispose()` to clean up** — Always call `vitis.dispose()` at the end of scripts to properly close connections.

---

## Quick Reference — Python APIs

### Project Management

| API | Description |
|-----|-------------|
| `vitis.create_client()` | Create a Vitis client instance |
| `client.set_workspace(path)` | Set workspace directory |
| `client.get_component(name)` | Get handle to existing component |
| `client.get_config_file(path)` | Get handle to configuration file |
| `client.find_platform_in_repos(name)` | Find platform XPFM path |
| `client.create_platform_component(...)` | Create platform |
| `client.create_app_component(...)` | Create application |
| `client.create_sys_project(...)` | Create system project |
| `client.create_aie_component(...)` | Create AI Engine component |
| `client.create_hls_component(...)` | Create HLS component |
| `vitis.dispose()` | Close all connections |

### XSDB Debug

| API | Description |
|-----|-------------|
| `start_debug_session()` | Create debug session |
| `s.connect(url)` | Connect to hw_server |
| `s.targets()` / `s.targets(n)` | List / select targets |
| `s.jtag_targets()` | List JTAG chain |
| `s.fpga(file)` | Program FPGA |
| `s.loadhw(hw)` | Load hardware description |
| `s.dow(file)` | Download ELF |
| `s.rst(type)` | Reset (processor/cores/system) |
| `s.con()` / `s.stop()` | Continue / stop |
| `s.stp()` / `s.nxt()` | Step into / step over |
| `s.bpadd(addr)` | Add breakpoint |
| `s.bt()` | Backtrace |
| `s.rrd()` / `s.mrd(addr)` | Read registers / memory |
| `s.mwr(addr, words)` | Write memory |
| `s.locals()` / `s.print(expr)` | Inspect variables |
| `s.memmap(file)` | Map symbol file for attach |
| `s.readjtaguart()` | Start JTAG UART capture |
| `s.jtagterminal()` | Start JTAG terminal |
| `s.svf()` | Create SVF generator |
